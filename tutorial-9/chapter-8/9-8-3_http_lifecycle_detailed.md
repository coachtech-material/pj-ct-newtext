...
- [ ] HTTPライフサイクルの全体像を理解している
- [ ] エントリーポイント（`public/index.php`）の役割を説明できる
- [ ] HTTPカーネルがミドルウェアを管理していることを理解している
- [ ] ミドルウェアの実行順序（リクエスト前処理→コントローラー→レスポンス後処理）を理解している
- [ ] ルーターがリクエストをコントローラーに振り分けることを理解している
- [ ] レスポンスの種類（HTML、JSON、リダイレクト、ファイルダウンロード）を理解している
- [ ] デバッグ時にHTTPライフサイクルの知識を活用できる
