...
| **ルーティング** | タッチパネル（注文端末） | お客さんの注文を受け付け、担当のホールスタッフに自動で通知する |
| **コントローラー** | ホールスタッフ | タッチパネルから通知を受け、厨房に伝え、出来た料理を運ぶ |
| **モデル** | シェフ | 冷蔵庫（DB）から食材を取り出し、調理してホールスタッフに渡す |
| **ビュー** | 盛り付け担当 | シェフから受け取った料理をお皿に綺麗に盛り付け、お客様に出せる状態にする |

> **📌 ポイント**
> 
> **ルーティングは「タッチパネル（注文端末）」**です。お客さんが注文すると、タッチパネルが自動的に担当のホールスタッフに通知します。
> 
...
タッチパネル: 「ユーザー一覧の注文が入りました」
              → 担当のホールスタッフ（UserController）に通知
```

通知を受けたホールスタッフ（コントローラー）が対応を開始します。

```
ホールスタッフ: 「ユーザー一覧ですね。シェフ、ユーザーデータをお願いします！」
```
...
**やっていること**: 「`/users`という注文が来たら、UserControllerのindexメソッドに通知する」という設定を定義しています。

レストランで言えば、**タッチパネルに「ユーザー一覧 → UserControllerのindexが担当」と登録してある**ようなものです。
...
> 「ルーティングはタッチパネル」「コントローラーはホールスタッフ」「モデルはシェフ」というイメージを持っていれば大丈夫です。
> 
