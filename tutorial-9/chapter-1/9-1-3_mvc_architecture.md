...
そこでLaravelは、「この種類の処理はここに書いてね」というルールを決めています。


Laravelは**設定より規約**という考え方を採用しています。
...
タッチパネル（ルーティング）が、注文内容を確認し、担当のホールスタッフ（コントローラー）に自動で通知します。
...
ホールスタッフ（コントローラー）は**司令塔**です。自分では料理（データ処理）も盛り付け（HTML生成）もしません。キッチンと盛り付け担当に指示を出すだけです。
...
キッチンは、データベースから必要なデータを取り出し、**調理（データの整形）**をしてホールスタッフ（コントローラー）に渡します。

---

### 4. 盛り付け（ビュー）

ホールスタッフ（コントローラー）は、キッチンから受け取った料理を盛り付け担当（ビュー）に渡します。
...
出来上がった料理（HTML）を、ホールスタッフ（コントローラー）がお客さんの元へ運びます。
...
**全部一人でやると、ちょっとした変更で負担が爆増するから**です。
...
居酒屋の例で言うと、お酒の種類を倍にしようとした時に、1人でお店を回している状態だと新しく覚えるのが困難ですが、ドリンク担当に分かれていると担当を1人増やせばいいなどの変更容易性があるといったイメージをすれば良いでしょう。
