...
{
    // 今回は「全員分取ってきて」という指示なので、
    // シェフは特別な調理法を書く必要がない
    // （基本の調理法は親方（Model）から受け継いでいる）
    
    // 例えば「VIPのお客様だけ取ってきて」のような
    // 特別な指示がある場合は、ここに調理法を書く
}
```
...
> **📌 なぜ中身が空なの？**
> 
> 今回の「全員分取ってきて」という指示は、シェフの基本スキルで対応できます。
> 基本スキルは親方（`Model`クラス）から受け継いでいるので、何も書かなくてOKです。
> 
> 「VIPだけ」「今月入会した人だけ」のような**特別な調理法**が必要な場合は、
> このファイルに書き足していきます。

**例：「VIPのお客様だけ取ってきて」という指示の場合**

```php
class User extends Model
{
    // 「VIPのお客様だけ取ってきて」という特別な調理法
...
> 
> ここでは雰囲気だけ掌んでください。
> 実際の書き方は、後のセクション（Eloquent）で詳しく学びます。
