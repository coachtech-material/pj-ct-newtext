...
### 📝 Step 5: `include`と`require`を使ったファイル読み込み
...
### 📝 Step 6: 実践例 - 3ステップの登録フォーム

学んだ内容を組み合わせて、入力→確認→完了の3ステップの登録フォームを作ってみましょう。

**入力画面（`step1_input.php`）**
...
<body>
    <h1>ユーザー登録（1/3）入力</h1>
    <form action="step2_confirm.php" method="post">
        <p>
            <label>お名前：</label>
...
<body>
    ...
    <form action="step3_complete.php" method="post">
        <!-- hidden属性で、データを隠して送信 -->
...
<body>
    ...
    <p><a href="step1_input.php">最初に戻る</a></p>
</body>
</html>
```
...
<input type="hidden" name="name" value="<?php echo htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?>">
```
→ `type="hidden"`は、画面には表示されないが、フォーム送信時にデータを送る隠しフィールドです。確認画面から完了画面へデータを渡すために使用します。

```php
<button type="button" onclick="history.back()">戻る</button>
```
→ `history.back()`はJavaScriptで、ブラウザの「戻る」ボタンと同じ動作をします。
