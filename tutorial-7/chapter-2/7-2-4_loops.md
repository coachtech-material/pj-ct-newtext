...
1.  **`break`**: ループを、**完全に中断**し、ループ構文の、直後から、処理を再開します。
...
- [ ] `for`ループを使って、指定した回数、処理を繰り返すことができる。
- [ ] `foreach`ループを使って、配列の、全ての要素を、取り出して、処理することができる。
- [ ] `break`と`continue`の、役割の違いを、説明できる。
- [ ] `for`と`foreach`の、どちらを使うべきか、状況に応じて、判断できる。
