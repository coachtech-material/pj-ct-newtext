# Tutorial 9-4-5: データの作成（create、save）

## 🎯 このセクションで学ぶこと

*   Eloquentを使って、データを作成できるようになる。
*   `create()`と`save()`の違いを理解し、使い分けられるようになる。
*   **`User::create()`（静的メソッド）と`$user->save()`（インスタンスメソッド）の違いを理解する。**
*   マスアサインメントの基本を理解する。
...
| `create([...])` | 配列のデータで新しいレコードを作成するメソッド |
| `$user` | 作成されたUserインスタンスを格納 |

> **💡 Tutorial 7の復習**: `::` は「クラスに直接アクセス」するための演算子です。`create()`はインスタンスを作らなくても呼び出せる**静的メソッド**です。
...
*   `create()`は、作成されたモデルの**インスタンス**を返します。
...
| 行 | コード | 意味 |
|:---:|:---|:---|
| 1 | `$user = new User();` | 新しいUserインスタンスを作成（まだDBには保存されていない） |
| 2 | `$user->name = '山田太郎';` | インスタンスのnameプロパティに値を代入 |
| 3 | `$user->email = 'taro@example.com';` | インスタンスのemailプロパティに値を代入 |
| 4 | `$user->password = Hash::make('password');` | インスタンスのpasswordプロパティに値を代入 |
| 5 | `$user->save();` | インスタンスをデータベースに保存 |

> **💡 Tutorial 7の復習**: `->` は「インスタンスのプロパティやメソッドにアクセス」するための演算子です。`$user`はインスタンスなので、`->`を使って`name`プロパティや`save()`メソッドにアクセスします。
...
| メソッド | 演算子 | 対象 | 呼び出し方 |
|:---|:---:|:---|:---|
| `create()` | `::` | **クラス**（静的メソッド） | `User::create([...])` |
| `save()` | `->` | **インスタンス**（インスタンスメソッド） | `$user->save()` |
...
| 行 | コード | 意味 |
|:---:|:---|:---|
| 1 | `$user = User::create([` | Userクラスのcreate()メソッドを呼び出し |
| 2 | `'name' => $validated['name'],` | nameカラムに値を設定 |
| 3 | `'email' => $validated['email'],` | emailカラムに値を設定 |
| 4 | `'password' => Hash::make($validated['password']),` | passwordカラムにハッシュ化した値を設定 |
| 5 | `]);` | 配列の終わり、レコードが作成される |
...
| 行 | コード | 意味 |
|:---:|:---|:---|
| 1 | `$user = new User();` | 空のUserインスタンスを作成 |
| 2-4 | `$user->xxx = ...;` | インスタンスのプロパティに値を代入 |
| 6-9 | `if (...) { ... }` | 条件に応じて追加のプロパティを設定 |
| 11 | `$user->save();` | 全ての設定が終わったらDBに保存 |

> **💡 ポイント**: `save()`を使うと、条件分岐などの複雑なロジックを挟んでから保存できます。
...
| 行 | コード | 意味 |
|:---|:---|:---|
| `protected $fillable = [...]` | マスアサインメントで代入可能なカラムを定義 |
| `'name', 'email', 'password'` | これらのカラムのみ`create()`で代入可能 |

これにより、`name`、`email`、`password`のみがマスアサインメントで代入可能になります。`is_admin`は含まれていないので、悪意のあるリクエストでも代入されません。
...
| 引数 | 意味 |
|:---|:---|
| 第1引数 `['email' => 'taro@example.com']` | 検索条件（このemailのユーザーを探す） |
| 第2引数 `['name' => ..., 'password' => ...]` | 見つからない場合に作成するデータ |
...
```php
// ❌ NG: create()はクラスの静的メソッドなので->は使えない
$user = User->create([...]);  // エラー！

// ✅ OK: クラスには::を使う
$user = User::create([...]);  // 正しい
```

#### 間違い4: `save()`に`::`を使う

```php
// ❌ NG: save()はインスタンスメソッドなので::は使えない
$user = new User();
$user::save();  // エラー！
...
| メソッド | 演算子 | 対象 | 特徴 |
|:---|:---:|:---|:---|
| `create()` | `::` | クラス（静的メソッド） | 配列を渡して一度に作成 |
| `save()` | `->` | インスタンス | インスタンスを作成してから保存 |
