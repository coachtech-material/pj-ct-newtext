...
#### `required` と `nullable` の違い

| ルール | 意味 | 空の値 | 値がない場合 |
|:---|:---|:---|:---|
| `required` | 必須入力 | エラー | エラー |
| `nullable` | 任意入力（nullを許可） | OK | OK |

**使い分け**: ユーザーが必ず入力すべき項目は`required`、入力しなくてもよい項目は`nullable`を使います。
...
データベースの値と照合するルールです。ユーザー登録時のメールアドレス重複チェックや、外部キーの存在確認などに使います。
...
| 書き方 | 意味 |
|:---|:---|
| `unique:users` | usersテーブルの同名カラム（email）で重複チェック |
| `unique:users,email` | usersテーブルのemailカラムで重複チェック |
| `unique:users,email,5` | ID=5のレコードを除外して重複チェック |

**更新時に自分自身を除外**

ユーザーがプロフィールを更新するとき、自分自身のメールアドレスは重複としてカウントしたくありません。
...
| 部分 | 意味 |
|:---|:---|
| `unique:users` | usersテーブルでチェック |
| `,email` | emailカラムを対象に |
| `,` . `$user->id` | このIDのレコードは除外（自分自身） |
...
「他のフィールドの値によって、必須かどうかを変えたい」場合に使います。

#### 条件付きルールの使い分け

| ルール | 意味 | 使用例 |
|:---|:---|:---|
| `required_if:A,値` | Aが指定値のとき必須 | 法人なら会社名必須 |
| `required_with:A` | Aが存在するとき必須 | 市区町村があれば番地も必須 |
| `required_without:A` | Aが存在しないとき必須 | 電話番号がなければメール必須 |
...
フォームで複数の値を送信する場合（タグの複数選択、複数ユーザーの一括登録など）は、配列のバリデーションが必要です。

#### `*`（ワイルドカード）とは？

`*`は「配列のすべての要素」を意味します。`tags.*`と書くと、`tags[0]`、`tags[1]`、`tags[2]`...のすべてに対してルールが適用されます。
...
| ルール | 意味 |
|:---|:---|
| `'tags' => 'required\|array'` | `tags`は必須で、配列であること |
| `'tags.*' => 'string\|max:50'` | `tags`の各要素は文字列で、50文字以内 |

**対応するリクエストデータの例**:

```php
// HTMLフォーム: <input name="tags[]" value="PHP">
// リクエストデータ:
...
複数ユーザーを一括登録するフォームなど、配列の中にオブジェクトがある場合です。
...
| ルール | 意味 |
|:---|:---|
| `'users' => 'required\|array'` | `users`は必須で、配列であること |
| `'users.*.name' => 'required\|string'` | 各ユーザーの`name`は必須で文字列 |
| `'users.*.email' => 'required\|email'` | 各ユーザーの`email`は必須でメール形式 |
