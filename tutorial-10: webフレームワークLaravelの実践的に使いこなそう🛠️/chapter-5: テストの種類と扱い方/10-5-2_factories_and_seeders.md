...
Laravelには、ファクトリー（Factorという仕組みがあり、テストデータを簡単に生成できます。
...
| ステップ | コード | 演算子 | 戻り値 | 意味 |
|:---:|:---|:---:|:---|:---|
| 1 | `User::factory()` | `::` | UserFactoryインスタンス | Userモデルのファクトリーを取得 |
| 2 | `->create()` | `->` | Userインスタンス | データベースに保存してインスタンスを返す |

> **💡 Tutorial 7の復習**: `User::factory()`は静的メソッドなので`::`を使います。このメソッドはファクトリーインスタンスを返すので、その後は`->`でインスタンスメソッドを呼び出します。
...
|:---:|:---|:---:|:---|:---|
| 1 | `User::factory()` | `::` | UserFactoryインスタンス | Userモデルのファクトリーを取得 |
| 2 | `->count(10)` | `->` | UserFactoryインスタンス | 作成数を10に設定 |
| 3 | `->create()` | `->` | Collection | 10件のユーザーを作成してコレクションで返す |

> **💡 ポイント**: `count()`はファクトリーインスタンスを返すので、続けて`->create()`を呼び出せます。このようにメソッドチェーンは、各メソッドが次のメソッドを呼び出せるオブジェクトを返すことで実現しています。
...
**コードリーディング（リレーション付きファクトリー）**

`User::factory()->has(Post::factory()->count(10))->create()` を分解してみましょう。

| ステップ | コード | 演算子 | 戻り値 | 意味 |
|:---:|:---|:---:|:---|:---|
| 1 | `User::factory()` | `::` | UserFactory | Userのファクトリーを取得 |
| 2 | `->has(...)` | `->` | UserFactory | 関連データを設定 |
| 3 | `Post::factory()` | `::` | PostFactory | Postのファクトリーを取得 |
| 4 | `->count(10)` | `->` | PostFactory | 10件作成する設定 |
| 5 | `->create()` | `->` | Userインスタンス | ユーザーと投稿10件を作成 |
