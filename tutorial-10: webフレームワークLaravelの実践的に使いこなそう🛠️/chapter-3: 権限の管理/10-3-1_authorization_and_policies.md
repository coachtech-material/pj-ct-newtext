...
前のチャプターで、Laravel Sanctumを使った認証（Authentication）を学びました。認証とは、「このユーザーは誰か」を識別する仕組みです。しかし、実際のWebアプリケーションでは、「誰か」を識別するだけでは不十分です。
...
このように、「誰がアクセスしているか」だけでなく、「そのユーザーが特定のアクションを実行する権限を持っているか」をチェックする必要があります。これが認可（Authorizatioです。
