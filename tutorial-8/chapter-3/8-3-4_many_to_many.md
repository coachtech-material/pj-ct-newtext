...
- [ ] 「多対多」リレーションシップがどのようなものか、投稿とタグの例で説明できる。
- [ ] なぜ中間テーブルが必要なのかを説明できる。
- [ ] 中間テーブルを作成するSQL文を書くことができ、2つの外部キーを設定できる。
- [ ] 複合ユニークキー制約の役割を理解し、なぜそれが必要なのかを説明できる。
- [ ] `INSERT`文を使って、中間テーブルにデータを登録し、多対多の関連を実際に作成できる。
