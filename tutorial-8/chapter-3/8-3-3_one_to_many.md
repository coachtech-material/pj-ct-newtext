...
# Tutorial 8-3-3: 1対多のリレーションシップ - 投稿テーブルを作成し、ユーザーと繋げよう
...
- [ ] `FOREIGN KEY`と`REFERENCES`を使ったSQL文を書き、1対多のテーブルを作成できる。
- [ ] 外部キー制約がなぜ必要なのかを、「データの整合性」という言葉を使って説明できる。
- [ ] `ON DELETE CASCADE`が何をするための設定か説明できる。
- [ ] 存在しない外部キーを持つデータを`INSERT`しようとするとエラーになることを、実際に試して確認した。
