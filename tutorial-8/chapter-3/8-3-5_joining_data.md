...
# Tutorial 8-3-5: JOIN句によるテーブルの結合 - 分割したデータをつなぎ合わせる
...
- [ ] `INNER JOIN`と`ON`を使って、1対多の関係にある2つのテーブルを結合し、目的のデータを取得できる。
- [ ] `INNER JOIN`と`LEFT JOIN`の違いを、「まだ投稿していないユーザー」の例を使って説明できる。
- [ ] `AS`を使ってテーブルにエイリアスを付け、SQLを簡潔に書くことができる。
- [ ] 3つ以上のテーブルを`JOIN`して、多対多の関係から必要な情報を取得するSQL文を書ける。
