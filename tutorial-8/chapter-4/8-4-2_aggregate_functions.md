...
- [ ] `COUNT`を使って、条件に合うレコードの件数を取得できる。
- [ ] `GROUP BY`を使って、ユーザーごとの投稿数など、グループごとの集計ができる。
- [ ] `WHERE`と`HAVING`の違いを、SQLの実行順序と関連付けて説明できる。
- [ ] 「投稿数が3件以上のユーザー」を`GROUP BY`と`HAVING`を使ってリストアップするSQL文を書ける。
