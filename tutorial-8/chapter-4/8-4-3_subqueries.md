...
- [ ] サブクエリがどのようなものか、自分の言葉で説明できる。
- [ ] `WHERE`句で`IN`とサブクエリを組み合わせ、特定の条件を満たすIDリストを使ってデータを絞り込める。
- [ ] `FROM`句に書いたサブクエリ（インラインビュー）に`AS`で別名を付け、メインクエリから参照できることを理解した。
- [ ] `EXISTS`が、関連データの存在チェックに使われることを理解した。
- [ ] 同じ結果を得るために、`JOIN`とサブクエリのどちらも使える場合があることを知っている。
