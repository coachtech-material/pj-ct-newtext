...
このように、`注文日`や、`商品名`などが、主キーの、**一部にしか、依存していない**状態を、**部分関数従属**と呼びます。これを、解消するために、テーブルを、分割します。
...
第3正規形の、ルールは、「**主キー以外の、カラムに、依存している、カラムを、別の、テーブルに、切り出す**」というものです。これを、**推移的関数従属**の、解消と呼びます。
...
- [ ] 正規化の、目的を、「データの、重複排除と、一貫性の、維持」という、言葉で、説明できる。
- [ ] 第1正規形から、第3正規形までの、ルールを、簡単な、言葉で、説明できる。
- [ ] ER図が、データベースの、「設計図」であることを、理解した。
- [ ] ER図に、描かれている、エンティティ、アトリビュート、リレーションシップが、それぞれ、何を、意味しているかを、説明できる。
- [ ] 簡単な、1対多の、リレーションシップを、ER図で、どのように、表現するかを、理解した。
""
