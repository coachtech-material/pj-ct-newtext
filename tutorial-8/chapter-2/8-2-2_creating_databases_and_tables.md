...
- [ ] `CREATE DATABASE` 文を、使って、指定した、文字コードで、データベースを、作成できる。
- [ ] `CREATE TABLE` 文の、基本的な、構文を、理解し、カラム名、データ型、簡単な、制約を、指定して、テーブルを、作成できる。
- [ ] `PRIMARY KEY`, `AUTO_INCREMENT`, `NOT NULL`, `UNIQUE` の、意味を、説明できる。
- [ ] phpMyAdminの、GUI操作でも、テーブルが、作成できることを、確認した。
""
