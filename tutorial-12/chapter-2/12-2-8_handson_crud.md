# Tutorial 12-2-8: ハンズオン - タスク管理APIのCRUD実装
...
```
~/api-practice/
├── 12-2-8_hands-on/                         ← このハンズオン用のディレクトリ
...
> 💡 **ポイント**: Chapter 1のハンズオン（12-1-7）とは別のプロジェクトを作成します。
...
cd ~/api-practice/12-2-8_hands-on/task-crud-practice
sail down
```
...
```bash
# ハンズオンディレクトリに移動
cd ~/api-practice/12-2-8_hands-on
