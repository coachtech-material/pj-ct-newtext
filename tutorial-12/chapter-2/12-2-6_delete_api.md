# Tutorial 12-2-6: DELETE APIの実装（削除）

## 🎯 このセクションで学ぶこと

*   タスクを削除するDELETE APIを実装する方法を学ぶ。
*   HTTPステータスコード（204 No Content、404 Not Found）を理解する。
*   なぜそのステータスコードを使うのかを学ぶ。
*   200 OKと204 No Contentの違いを理解する。

---

## 🧠 先輩エンジニアの思考プロセス

### 「なぜ更新APIの後に『削除API』を実装するのか？」

更新APIができたら、次は「削除API」です。

---

### 理由1: CRUDの最後

CRUD（Create, Read, Update, Delete）の最後は「削除」です。

これでCRUD APIが完成します。

---

### 理由2: 削除は慎重に

削除は**取り消しができない**操作です。

- 本当に削除していいか確認
- 論理削除（soft delete）の検討
- 関連データの処理

---

### 理由3: 適切なレスポンス

削除成功時は、**204 No Content**を返すのが一般的です。

```php
$task->delete();
return response()->json(null, 204);
```

---

### このセクションでやること

| 順番 | 作業 | 理由 |
|------|------|------|
| 1 | destroyメソッドの実装 | 削除のロジック |
| 2 | 存在確認 | 削除対象が存在するか |
| 3 | 適切なレスポンス | 204ステータスコードを返す |

> 💡 **ポイント**: DELETEリクエストは「削除」に使います。

---

## 導入：なぜ204 No Contentを使うのか

リソースを削除した場合、**200 OK**ではなく、**204 No Content**を返すのが一般的です。

*   **200 OK**: リクエストが成功した（レスポンスボディあり）
*   **204 No Content**: リクエストが成功した（レスポンスボディなし）

**204 No Content**を使うことで、**レスポンスボディが不要**であることを明確に伝えられます。

---

## 詳細解説

### 🔍 タスクを削除するAPI

タスクを削除するAPIを実装します。

**エンドポイント**: `DELETE /api/tasks/{id}`

**レスポンス例（成功時）**:

*   ステータスコード: `204 No Content`
*   ボディ: なし

---

### 🔍 コントローラーの実装

**ファイル**: `app/Http/Controllers/Api/TaskController.php`

```php
public function destroy($id)
{
    $task = Task::find($id);
    
    if (!$task) {
        return response()->json([
            'message' => 'Task not found'
        ], 404);
    }
    
    $task->delete();
    
    return response()->json(null, 204);
}
```

**ポイント**:
*   `Task::find($id)`で、タスクを取得する
*   タスクが見つからない場合は、**404 Not Found**を返す
*   `$task->delete()`で、タスクを削除する
*   **204 No Content**を返す
*   レスポンスボディは`null`

---

### 🔍 HTTPステータスコード: 204 No Content

**204 No Content**は、**リクエストが成功したが、レスポンスボディがない**ことを示します。

**いつ使うか**:
*   リソースが正常に削除された
*   処理が正常に完了したが、返すデータがない

**なぜ204を使うのか**:
*   クライアント側で「削除が成功した」と判断できる
*   レスポンスボディが不要であることを明確に伝えられる
*   帯域幅を節約できる

**例**:

```php
return response()->json(null, 204);
```

または、

```php
return response()->noContent();
```

---

### 🔍 HTTPステータスコード: 404 Not Found

**404 Not Found**は、**リソースが見つからなかった**ことを示します。

**いつ使うか**:
*   指定されたIDのリソースが存在しない

**なぜ404を使うのか**:
*   クライアント側で「リソースが存在しない」と判断できる
*   ユーザーに適切なエラーメッセージを表示できる

**例**:

```php
return response()->json([
    'message' => 'Task not found'
], 404);
```

---

### 🔍 200 OKと204 No Contentの違い

| ステータスコード | レスポンスボディ | 用途 |
|----------------|----------------|------|
| **200 OK** | あり | 削除されたリソースのデータを返す |
| **204 No Content** | なし | 削除が成功したことだけを伝える |

**推奨**: DELETEリクエストでは、**204 No Content**を使う

---

### 🔍 200、204、404の使い分け

| ステータスコード | 状況 | レスポンスボディ |
|----------------|------|----------------|
| **200 OK** | タスクが削除された（データを返す） | 削除されたタスクのデータ |
| **204 No Content** | タスクが削除された（データを返さない） | なし |
| **404 Not Found** | タスクが見つからなかった | エラーメッセージ |

---

### 🔍 実践演習: タスク削除APIをテストしてください

Thunder Clientで、以下のリクエストを送信してください。

**1. 成功する場合**

*   メソッド: `DELETE`
*   URL: `http://localhost:8000/api/tasks/1`

**期待されるレスポンス**:

*   ステータスコード: `204 No Content`
*   ボディ: なし

---

**2. タスクが見つからない場合**

*   メソッド: `DELETE`
*   URL: `http://localhost:8000/api/tasks/9999`

**期待されるレスポンス**:

*   ステータスコード: `404 Not Found`
*   ボディ: `{"message": "Task not found"}`

---

### 🔍 response()->noContent()を使う

Laravelでは、`response()->noContent()`を使うと、204 No Contentを返せます。

```php
public function destroy($id)
{
    $task = Task::findOrFail($id);
    $task->delete();
    
    return response()->noContent();
}
```

**ポイント**:
*   `response()->noContent()`は、`response()->json(null, 204)`と同じ

---

### 🔍 削除されたリソースのデータを返す場合

削除されたリソースのデータを返す場合は、**200 OK**を使います。

```php
public function destroy($id)
{
    $task = Task::findOrFail($id);
    $deletedTask = $task->toArray();
    $task->delete();
    
    return response()->json($deletedTask, 200);
}
```

**ポイント**:
*   削除前にデータを取得しておく
*   削除後にデータを返す

---

### 🔍 ステータスコードの重要性

**適切なステータスコードを返す**ことで、クライアント側で以下のことができます。

*   **成功・失敗を判断する**: 204なら削除成功、404ならリソースが存在しない
*   **エラー処理を行う**: 404の場合は「タスクが見つかりません」と表示
*   **リトライを判断する**: 404の場合はリトライしない

---

### 🔍 クライアント側での処理例（JavaScript）

```javascript
fetch('http://localhost:8000/api/tasks/1', {
  method: 'DELETE',
})
  .then(response => {
    if (response.status === 204) {
      console.log('削除されました');
    } else if (response.status === 404) {
      return response.json().then(data => {
        throw new Error(data.message);
      });
    }
  })
  .catch(error => {
    console.error('エラー:', error);
  });
```

---

### 💡 TIP: ソフトデリート

**ソフトデリート**を使うと、データを物理的に削除せず、論理的に削除できます。

**マイグレーション**:

```php
$table->softDeletes();
```

**モデル**:

```php
use Illuminate\Database\Eloquent\SoftDeletes;

class Task extends Model
{
    use SoftDeletes;
}
```

**削除**:

```php
$task->delete(); // deleted_atに日時が設定される
```

**完全削除**:

```php
$task->forceDelete(); // データベースから完全に削除される
```

---

### 🚨 よくある間違い

#### 間違い1: 200 OKを返す

削除が成功した場合は、**204 No Content**を返すのが推奨されます。

```php
// ❌ 間違い（推奨されない）
return response()->json(['message' => 'Deleted'], 200);

// ✅ 正しい（推奨）
return response()->noContent();
```

---

#### 間違い2: レスポンスボディを返す

204 No Contentの場合は、レスポンスボディを返しません。

```php
// ❌ 間違い
return response()->json(['message' => 'Deleted'], 204);

// ✅ 正しい
return response()->json(null, 204);
// または
return response()->noContent();
```

---

#### 間違い3: 存在しないタスクを削除しようとした場合に200を返す

タスクが見つからない場合は、**404 Not Found**を返します。

```php
// ❌ 間違い
public function destroy($id)
{
    $task = Task::find($id);
    
    if (!$task) {
        return response()->json([
            'message' => 'Task not found'
        ], 200); // 200は間違い
    }
    
    $task->delete();
    return response()->noContent();
}

// ✅ 正しい
public function destroy($id)
{
    $task = Task::find($id);
    
    if (!$task) {
        return response()->json([
            'message' => 'Task not found'
        ], 404); // 404が正しい
    }
    
    $task->delete();
    return response()->noContent();
}
```

---

## ✨ まとめ

このセクションでは、タスクを削除するDELETE APIを実装しました。

*   **204 No Content**: タスクが削除された場合に返す（レスポンスボディなし）
*   **404 Not Found**: タスクが見つからなかった場合に返す
*   200 OKと204 No Contentの違いを理解した

次のセクションでは、親切なエラーレスポンスの作成について学びます。

---

## 📝 学習のポイント

- [ ] タスクを削除するAPIを実装した。
- [ ] 204 No Contentを理解した。
- [ ] 404 Not Foundを理解した。
- [ ] 200 OKと204 No Contentの違いを理解した。
- [ ] response()->noContent()を使った。
