...
`docs/manual-work.md` の4項目め、「同じ受け入れ条件を、自分で確かめるときとAIに回させるときの2回、貼り直した」を部品にします。UAT（User Acceptance Test＝受け入れテスト）は、使う人と同じように画面を触って、決めた条件どおりかを見る確認です（14-4-3）。

14-4-4でAIに回させたとき、依頼文には受け入れ条件のほかに、ログイン情報・開くURL・報告のしかたが入っていました。毎回変わるのは受け入れ条件だけで、残りは固定です。固定の部分をSkillに移し、変わる部分だけを渡す形にします。
...
3. 【自分⇄AI】「いまこの会話でやった手順をSkillにして」と送り、聞き返しに答える
4. 【自分】できた `SKILL.md` を全文読み、固定するはずの4つが入っているかを確かめる
5. 【自分】受け入れ条件の受け取り口を、自分の手で書き足す
6. 【自分⇄AI】`/uat` に `docs/reply.md` の受け入れ条件を渡して回し、2の結果と見比べる
7. 【自分】`git status` を見てから、`main` へコミットしてpushする

### ✅ 完成チェックリスト

- [ ] `/uat` を呼ぶと、渡した受け入れ条件の行ごとに判定が返る（リプライなら4行）
- [ ] 判定の行に、そう判断した理由とスクリーンショットが対応している
- [ ] タスク2で回したときと同じ4行に、同じ判定が返っている
- [ ] `SKILL.md` を読むと、渡した受け入れ条件がどこで使われるのかが分かる
- [ ] `SKILL.md` にログイン情報・URL・報告のしかた・ファイルを残さないことが書かれている
- [ ] `git status` に出るのが `.claude/` の下だけで、画像のファイルが増えていない
...
**スクリーンショットが作業フォルダに落ちるとき**

保存先は環境で変わります。リポジトリの中に残っていないことだけを、Step 6で確かめます。
...
`.claude/skills/uat/SKILL.md` をVS Codeで開き、固定するはずの4つが入っているかを確かめます。落ちているものは、自分で書き足します。
...
```text
Claude Code: （Skill(uat) の行が出たあと、docs/reply.md を読み、ブラウザで
...
Step 2の報告と見比べます。同じ4行に、同じ判定が返っていれば、依頼文を貼り直す作業が部品に移っています。
...
開いて読むと、URLと報告のしかたは入っていましたが、受け入れ条件がリプライの4行のまま埋め込まれていました。この部品はほかの機能でも呼ぶので、その4行を消して `$ARGUMENTS` に置き換え、「ファイルの場所を渡されたら開いて読む」の2行を足しました。

**5. 呼んでみる**

`/uat docs/reply.md に書いたリプライ機能の受け入れ条件` と打つと、`Skill(uat)` の行が出て、ブラウザが動き始めました。返ってきた報告は4行で、Step 2と同じ判定でした。
