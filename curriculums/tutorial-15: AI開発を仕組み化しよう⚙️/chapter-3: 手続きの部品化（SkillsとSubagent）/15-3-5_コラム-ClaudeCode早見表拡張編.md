...
同梱の `/design-sync [hint]` を打つと、リポジトリの中のReactのデザインシステムを Claude Design に取り込みます。取り込んでおくと、Claude Designが作るデザインが、実際に存在するコンポーネントを使ったものになります。初めて取り込むときは、大きいリポジトリだと数時間かかることがあります。

このアプリはReactを使っていないので、この教材では扱いません。設計をコードの外に置いて、そこから作らせるという考え方は、Tutorial 13から続けてきたものと同じです。置く先が設計書だけでなく、デザインシステムにも広がっている、と見てください。
