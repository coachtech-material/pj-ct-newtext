...
- worktreeが何かを理解し、`claude -w` で書く場所をもう1つ作れるようになる。
...
worktreeは、同じリポジトリを別のフォルダにもう1つ展開したものです（14-1-7）。それぞれのフォルダが自分のファイルと自分のブランチを持ち、履歴とリモートだけを元のフォルダと共有します。

1つのフォルダで開けるブランチは、いつでも1本だけです。2本目の作業を始めるには、いまの作業をコミットするか、いったん片づけてから切り替えることになります。worktreeを作れば、2本目は別のフォルダで開くので、どちらも開いたままにできます。
...
worktreeは新しくチェックアウトしたフォルダなので、入っているのは**Gitで管理しているファイルだけ**です。このアプリで管理していないものは `.gitignore` に書いてあり、その中に `/vendor` と `.env` の2つがあります。つまり書く場所には `vendor/bin/sail` が無く、`.env` も無いので、**アプリもテストも動きません**。
...
書く場所のセッションから元のフォルダのファイルを編集したり、元のフォルダでコマンドを動かしたりしようとすると、Claude Codeがその操作を止めます。うっかり混ざる心配は要りません。
...
あわせて .gitignore に .claude/worktrees/ を1行足して。
どちらもコミットして、main に push して。
```
...
`.claude/worktrees/edited-mark/` にいて、`vendor` と `.env` は無く、決まりとSkillはある、という答えが返れば、分担のとおりです。
...
- 書く場所にはGitで管理しているファイルだけが入る。`vendor/` と `.env` が無いので、そこではアプリもテストも動かない。実装の依頼に「テストは書くだけ」の1行を足す。
