...
このチャプターでは、APIの基本概念やJSON、開発ツール、ルーティングについて学びます。コードやコマンドが出てきますが、まずは「APIがどのように動くのか」を理解しながら読み進めてください。実際に手を動かすのは、チャプターの最後にあるハンズオン（11-1-7）で行います。
...
Tutorial 10でWebアプリケーションを完成させました。次は「API開発」です。
...
Tutorial 10で学んだWeb認証（Fortify）と、API開発で使うAPI認証（Token）は、仕組みが大きく異なります。
