...
- `~/api-practice/11-1-7_hands-on/api-setup-practice/` ディレクトリの中身（Laravelプロジェクト一式）を **publicリポジトリ** で管理する
- リポジトリ名は **`api-setup-practice`** とする
- 下記の雛形をもとに、`README.md` を **自分の言葉で** 作成する
- コミットとpushを完了させる

> 💡 **`practice/` と `sample/` の使い分け**:
> - **`api-setup-practice/`** が **「提出物」** です。最終的にここにある成果物をGitHubに push します
> - **`api-setup-sample/`** は **「答え合わせ用」** で、ローカルでの比較確認のみに使います。GitHubには push しません

<details>
<summary>📄 README.md の雛形（クリックで展開）</summary>

`api-setup-practice/README.md` に、以下の雛形をベースに **自分の言葉で** 記載しましょう（Laravelデフォルトの README を置き換えてOK）。
...
> 💡 **「学んだこと」の書き方の例（参考）**:
> - REST API の基本概念（リソース、HTTPメソッド、ステータスコード）
> - Laravel での API ルート（`routes/api.php`）の書き方
> - Postman を使った API リクエスト送信とレスポンス確認
...
- [ ] GitHub に `api-setup-practice` リポジトリを作成した（**public** で作成）
