...
<img alt="11-1-6_c1.png" src="https://s3.ap-northeast-1.amazonaws.com/coachtech-lms-bucket-dev/curriculums/images/11-1-6_c1.png">
...
<img alt="11-1-6_c2.png" src="https://s3.ap-northeast-1.amazonaws.com/coachtech-lms-bucket-dev/curriculums/images/11-1-6_c2.png">
...
2-3と2-4では、開発環境と本番環境の設定例を紹介しました。しかし、実務では**環境ごとにコードを書き換える**のは現実的ではありません。

そこで、**環境変数を使って設定を切り替える**方法を使います。

---

#### FRONTEND_URLを.envに追加する

`FRONTEND_URL`は**Laravelにデフォルトで用意されている環境変数ではありません**。自分で`.env`ファイルに追加する設定項目です。
...
`env('FRONTEND_URL', 'http://localhost:3000')`の意味は以下の通りです。

| 部分 | 説明 |
|------|------|
| `env('FRONTEND_URL', ...)` | `.env`から`FRONTEND_URL`の値を取得 |
| `'http://localhost:3000'` | `.env`に設定がない場合のデフォルト値 |
...
|------|----------|------------|
| 2-3（`'*'`） | 設定が簡単 | セキュリティリスク |
| 2-4（直接記述） | セキュリティ◎ | 環境ごとにコード変更が必要 |
| 2-5（環境変数） | セキュリティ◎、コード変更不要 | `.env`の管理が必要 |

実務では、**コードは同じまま、`.env`ファイルだけを環境ごとに変える**のが一般的です。
...
**問題**: CORSエラーは**ブラウザが表示するエラー**です。サーバーは正常にレスポンスを返していますが、ブラウザがCORSポリシーに基づいてレスポンスをブロックしています。

| 視点 | 状態 |
|------|------|
| サーバー | 正常にレスポンスを返している（200 OK） |
| ブラウザ | CORSヘッダーがないため、レスポンスをブロック |

**対処法**: サーバー側で`Access-Control-Allow-Origin`ヘッダーを返すように設定します（`config/cors.php`）。
...
次のセクションでは、ここまでのChapter 1で学んだAPI開発の基礎を実践するハンズオン演習に進みます。
