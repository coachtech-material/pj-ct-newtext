# Tutorial 11-2-1: タスクAPIの実装（CRUD）

## 📖 このチャプターの進め方

このチャプターでは、CRUD操作やAPIリソース、エラーレスポンスなど、様々なAPI処理の実装方法を学びます。コードが出てきますが、まずは「各API処理がどのように実装されるか」を理解しながら読み進めてください。実際に手を動かすのは、チャプターの最後にあるハンズオン（11-2-8）で行います。
