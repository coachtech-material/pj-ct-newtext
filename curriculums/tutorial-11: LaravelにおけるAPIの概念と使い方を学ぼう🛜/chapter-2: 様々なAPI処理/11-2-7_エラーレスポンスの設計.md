...
ここでは、コントローラーを書きながらエラーの形を決めました。この形を実装の前に一覧として決めておくのが、13-4-1「API設計」で作るエラーレスポンス一覧です。どのエンドポイントが、どのステータスコードで、どんな中身を返すのかを先に置いておくと、画面を作る人はサーバー側のコードを開かずに済みます。

「どんな異常がありうるか」を洗い出す作業は、さらにその手前にあります。13-6-3「例外設計」で扱います。
