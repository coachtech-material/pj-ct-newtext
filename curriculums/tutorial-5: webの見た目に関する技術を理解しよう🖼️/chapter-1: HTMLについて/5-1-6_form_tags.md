...
*   **`id` 属性**: HTML要素に付ける**ユニークな名前**です。同じページ内で、同じ `id` を持つ要素は一つだけでなければなりません。
*   **`for` 属性**: `<label>` に付ける属性で、「この名札は、`id` が○○の入力フィールドのものですよ」と指定します。

`for` の値と `id` の値を**同じ文字列**にすることで、ラベルと入力フィールドが関連付けられます。
...
フォームの入力フィールドに `required` 属性を付けると、そのフィールドが**必須項目**になります。必須項目が空のまま送信ボタンを押すと、ブラウザが自動的に「このフィールドを入力してください」という警告を表示し、送信を止めてくれます。
...
*   **`required`**: この属性には値を指定する必要はありません。属性名を書くだけで、そのフィールドが必須になります。
*   `<input>`、`<textarea>`、`<select>` など、多くの入力要素に使えます。
