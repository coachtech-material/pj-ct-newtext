...
このチャプターでは、ミドルウェアの仕組みやカスタムミドルウェアの作成方法について学びます。コードやコマンドが出てきますが、まずは「リクエストの流れにどう処理を挟むのか」を理解しながら読み進めてください。実際に手を動かすのは、チャプターの最後にあるハンズオン（10-2-4）で行います。
