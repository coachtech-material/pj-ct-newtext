...
- `~/laravel-practice/10-2-4_hands-on/middleware-app-practice/` ディレクトリの中身（Laravelプロジェクト一式）を **publicリポジトリ** で管理する
- リポジトリ名は **`middleware-app-practice`** とする
- 下記の雛形をもとに、`README.md` を **自分の言葉で** 作成する
- コミットとpushを完了させる

> 💡 **`practice/` と `sample/` の使い分け**:
> - **`middleware-app-practice/`** が **「提出物」** です。最終的にここにある成果物をGitHubに push します
> - **`middleware-app-sample/`** は **「答え合わせ用」** で、ローカルでの比較確認のみに使います。GitHubには push しません

<details>
<summary>📄 README.md の雛形（クリックで展開）</summary>

`middleware-app-practice/README.md` に、以下の雛形をベースに **自分の言葉で** 記載しましょう（Laravelデフォルトの README を置き換えてOK）。
...
> 💡 **「学んだこと」の書き方の例（参考）**:
> - ミドルウェアが Request と Response の間で処理を介在させる仕組み
> - `sail artisan make:middleware` でのミドルウェア作成と Kernel.php での登録
> - ルートへのミドルウェア適用方法（`->middleware('check_admin')`）
...
```bash
# middleware-app-practice ディレクトリに移動
cd ~/laravel-practice/10-2-4_hands-on/middleware-app-practice
...
- [ ] GitHub に `middleware-app-practice` リポジトリを作成した（**public** で作成）
