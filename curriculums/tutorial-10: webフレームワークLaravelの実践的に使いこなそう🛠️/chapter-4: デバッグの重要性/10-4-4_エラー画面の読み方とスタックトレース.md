...
検索と同じことは、AIにも頼めます。1-1-6で決めたとおり、この段階で頼むのは「原因として考えられる可能性」と「次に試すこと」までです。修正済みのコードをもらうと、どの行が効いたのか分からないまま画面が動いてしまいます。挙がった候補は、自分で1つずつ当てて確かめてください。

渡すのは、エラー画面のいちばん上のメッセージ、その下のファイル名と行番号、そして自分が書いたコードです。スタックトレースの `vendor/` の行は、貼っても手がかりになりません。`app/` から始まる行だけを選んで貼ります。

何度聞いても直らないときに、どこで打ち切って別の手に切り替えるかは、AIに実装を任せるようになってから効いてきます。14-5-1「直らないときの型」で扱います。
...
*   AIには「原因として考えられる可能性」と「次に試すこと」を聞き、直すのは自分の手で行う。
