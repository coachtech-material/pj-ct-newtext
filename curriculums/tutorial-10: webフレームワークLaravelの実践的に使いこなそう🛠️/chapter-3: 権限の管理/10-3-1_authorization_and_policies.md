...
このチャプターでは、ポリシーやゲートを使った権限管理の仕組みについて学びます。コードやコマンドが出てきますが、まずは「誰に何を許可するか」をどう設計するかを理解しながら読み進めてください。実際に手を動かすのは、チャプターの最後にあるハンズオン（10-3-5）で行います。
...
Chapter 1で、Laravel Fortifyを使った**認証（Authentication）**を学びました。認証とは、「このユーザーは誰か」を識別する仕組みです。しかし、実際のWebアプリケーションでは、「誰か」を識別するだけでは不十分です。
...
ゲートは、`app/Providers/AuthServiceProvider.php`の`boot`メソッドで定義します。
