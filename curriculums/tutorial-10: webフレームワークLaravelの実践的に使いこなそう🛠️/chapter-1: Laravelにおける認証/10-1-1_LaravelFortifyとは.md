# 10-1-1: Laravel Fortifyとは

## 📖 このチャプターの進め方

このチャプターでは、Laravel Fortifyを使った認証機能の仕組みについて学びます。コードやコマンドが出てきますが、まずは「認証がどのように動くのか」を理解しながら読み進めてください。実際に手を動かすのは、チャプターの最後にあるハンズオン（10-1-6）で行います。
...
このChapterでは、Laravel Fortifyを使った認証機能について学びます。各セクションで以下の内容を扱います。

```
10-1-1 Laravel Fortifyとは（このセクション）
  └─ Fortifyの概要、セッションベース認証の仕組み
       ↓
10-1-2 Fortifyの認証機能を理解する
  └─ インストール、設定ファイル、FortifyServiceProvider
       ↓
10-1-3 ユーザー登録機能を理解する
  └─ 登録フォーム（register.blade.php）、CreateNewUserアクション
       ↓
10-1-4 ログイン・ログアウト機能を理解する
  └─ ログインフォーム（login.blade.php）、ダッシュボード（dashboard.blade.php）、ログアウト
       ↓
10-1-5 認証済みユーザーの取得とミドルウェア
  └─ auth()ヘルパー、authミドルウェア、@auth/@guestディレクティブ
       ↓
10-1-6 ハンズオン：認証機能の実装
  └─ 実際に手を動かして認証機能を構築
```
...
これまでのセクションで、データベース操作、バリデーション、リレーションシップなど、Laravelの基本的な機能を学んできました。しかし、実際のWebアプリケーションでは、「誰がアクセスしているのか」を識別する仕組みが必要です。これが認証（Authentication）です。

<img alt="10-1-1_c1.png" src="https://s3.ap-northeast-1.amazonaws.com/coachtech-lms-bucket-dev/curriculums/images/10-1-1_c1.png">
...
*   **認証（Authentication）**：「誰がアクセスしているか」を確認すること（このチャプターで学ぶ）
