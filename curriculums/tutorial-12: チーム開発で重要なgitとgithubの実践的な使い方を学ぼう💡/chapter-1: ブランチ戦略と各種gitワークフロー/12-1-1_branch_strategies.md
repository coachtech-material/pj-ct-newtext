...
このチャプターでは、ブランチ戦略やブランチ操作、マージ、コンフリクト解消について学びます。コマンドが出てきますが、まずは「チーム開発でどのようにブランチを運用するか」を理解しながら読み進めてください。実際に手を動かすのは、チャプターの最後にあるハンズオン（12-1-5）で行います。
