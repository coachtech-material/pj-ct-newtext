...
このチャプターでは、バリデーションの仕組みやルール、カスタムバリデーション、フォームリクエストについて学びます。コード例が多く出てきますが、まずは「バリデーションはこう実装するんだな」と理解しながら読み進めてください。実際にフォームにバリデーションを組み込むのは、チャプターの最後にあるハンズオン（9-6-5）で行います。
...
バリデーションが失敗した場合、Laravelは自動的に前のページにリダイレクトし、エラーメッセージを`$errors`変数に格納します。
