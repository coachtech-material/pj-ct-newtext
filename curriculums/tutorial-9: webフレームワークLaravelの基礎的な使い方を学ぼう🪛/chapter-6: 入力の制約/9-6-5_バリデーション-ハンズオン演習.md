...
- `~/laravel-practice/9-6-5_hands-on/validation-app-practice/` ディレクトリの中身（Laravelプロジェクト一式）を **publicリポジトリ** で管理する
- リポジトリ名は **`validation-app-practice`** とする
- 下記の雛形をもとに、`README.md` を **自分の言葉で** 作成する
- コミットとpushを完了させる

> 💡 **`practice/` と `sample/` の使い分け**:
> - **`validation-app-practice/`** が **「提出物」** です。最終的にここにある成果物をGitHubに push します
> - **`validation-app-sample/`** は **「答え合わせ用」** で、ローカルでの比較確認のみに使います。GitHubには push しません

<details>
<summary>📄 README.md の雛形（クリックで展開）</summary>

`validation-app-practice/README.md` に、以下の雛形をベースに **自分の言葉で** 記載しましょう（Laravelデフォルトの README を置き換えてOK）。
...
> 💡 **「学んだこと」の書き方の例（参考）**:
> - `$request->validate()` を使ったバリデーションルール（`required`, `email`, `unique`, `min`）の指定方法
> - エラーメッセージの日本語化と、フィールド毎のカスタマイズ
> - Blade での `@error` ディレクティブと `old()` ヘルパーによる入力値の復元
...
```bash
# validation-app-practice ディレクトリに移動
cd ~/laravel-practice/9-6-5_hands-on/validation-app-practice
...
- [ ] GitHub に `validation-app-practice` リポジトリを作成した（**public** で作成）
