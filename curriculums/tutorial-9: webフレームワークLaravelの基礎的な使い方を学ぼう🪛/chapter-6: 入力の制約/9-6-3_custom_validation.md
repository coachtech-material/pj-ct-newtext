...
最も簡単な方法は、**クロージャ（無名関数）**を使う方法です。1箇所だけで使う簡易的なカスタムバリデーションに適しています。

#### クロージャとは？

クロージャとは、名前を持たない関数のことです。変数に代入したり、他の関数の引数として渡したりできます。
...
バリデーションでクロージャを使うと、**その場で独自のルールを定義**できます。

#### なぜ配列形式で書くのか？

クロージャを含めるときは、ルールを**配列形式**で指定する必要があります。
...
// 配列形式（クロージャ可）
'name' => ['required', 'string', 'max:255', function (...) { ... }]
```
...
| 部分 | 意味 |
|:---|:---|
| `'username' => [...]` | 配列形式でルールを指定 |
| `'required', 'string'` | 標準ルール（必須、文字列） |
| `function ($attribute, $value, $fail)` | カスタムルールのクロージャ |
| `strtolower($value)` | 入力値を小文字に変換 |
| `$fail('...')` | バリデーション失敗、エラーメッセージを設定 |
...
| `$value` | 入力された値 | `'Admin'` |
| `$fail` | 失敗時にエラーメッセージを設定する関数 | `$fail('エラーです')` |

> 💡 **ポイント**: `$fail()`を呼び出すとバリデーションが失敗します。呼び出さなければ成功とみなされます。
...
再利用可能なカスタムバリデーションルールを作成するには、**Ruleクラス**を使います。複数の場所で同じバリデーションを使う場合は、こちらが推奨です。

#### Ruleクラスのメリット

| メリット | 説明 |
|:---|:---|
| 再利用可能 | 複数のコントローラーで同じルールを使える |
| テストしやすい | 単体テストでルールだけをテストできる |
| 管理しやすい | `app/Rules/`ディレクトリにまとまる |

#### 実装する2つのメソッド

| メソッド | 役割 | 返り値 |
|:---|:---|:---|
| `passes($attribute, $value)` | バリデーションロジック | `true`で成功、`false`で失敗 |
...
カスタムルールを**グローバルに登録**して、アプリケーション全体で使い回すこともできます。サービスプロバイダに一度登録するだけで、どのコントローラーやフォームリクエストからでも通常のバリデーションルールと同じように呼び出せるため、複数の画面で同じルールを使いたい場合に便利です。
