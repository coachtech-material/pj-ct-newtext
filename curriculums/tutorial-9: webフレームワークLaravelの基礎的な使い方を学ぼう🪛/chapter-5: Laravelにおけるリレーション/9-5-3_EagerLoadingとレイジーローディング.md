# 9-5-3: Eager Loadingとレイジーローディング
...
**クエリを発行する前に、読み込むリレーションがわかっている場合**に使います。典型的なのは、一覧画面や詳細画面で関連データを表示するケースです。
...
**クエリを発行した後に、追加でリレーションを読み込みたい場合**に使います。例えば、条件によって読み込むリレーションが変わる場合に便利です。
...
**複数の処理を経由するときに、重複クエリを防ぎたい場合**に使います。例えば、あるサービスクラスで`user`を読み込み、別のサービスクラスで`user`と`comments`を読み込む場合、`loadMissing()`なら`user`は再読み込みされません。
...
**リレーション先のデータ自体は不要で、件数だけ知りたい場合**に使います。例えば、一覧画面で「コメント数」を表示したいが、コメントの内容は不要な場合です。`with('comments')`で全データを読み込むより、はるかに効率的です。
...
ブラウザで画面を開いた場合、画面下部にデバッグバーが表示されます。「Queries」タブをクリックすると、実行されたSQLクエリの数が表示されます。
