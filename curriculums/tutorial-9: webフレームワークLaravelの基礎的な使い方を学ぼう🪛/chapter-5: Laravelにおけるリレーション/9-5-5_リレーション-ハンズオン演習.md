...
- `~/laravel-practice/9-5-5_hands-on/relation-app-practice/` ディレクトリの中身（Laravelプロジェクト一式）を **publicリポジトリ** で管理する
- リポジトリ名は **`relation-app-practice`** とする
- 下記の雛形をもとに、`README.md` を **自分の言葉で** 作成する
- コミットとpushを完了させる

> 💡 **`practice/` と `sample/` の使い分け**:
> - **`relation-app-practice/`** が **「提出物」** です。最終的にここにある成果物をGitHubに push します
> - **`relation-app-sample/`** は **「答え合わせ用」** で、ローカルでの比較確認のみに使います。GitHubには push しません

<details>
<summary>📄 README.md の雛形（クリックで展開）</summary>

`relation-app-practice/README.md` に、以下の雛形をベースに **自分の言葉で** 記載しましょう（Laravelデフォルトの README を置き換えてOK）。
...
> 💡 **「学んだこと」の書き方の例（参考）**:
> - 1対多リレーション（`hasMany` / `belongsTo`）と多対多リレーション（`belongsToMany`）の定義
> - リレーション越しのデータアクセス（`$post->comments` など）
> - Eager Loading（`with()`）でN+1問題を回避する仕組み
...
```bash
# relation-app-practice ディレクトリに移動
cd ~/laravel-practice/9-5-5_hands-on/relation-app-practice
...
- [ ] GitHub に `relation-app-practice` リポジトリを作成した（**public** で作成）
