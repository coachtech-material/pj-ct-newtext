...
このチャプターでは、Eloquentを使ったリレーションシップの定義とデータ取得の方法を学びます。コード例が多く出てきますが、まずは「リレーションはこう定義して使うんだな」と理解しながら読み進めてください。実際にリレーションを使ったCRUD機能を実装するのは、チャプターの最後にあるハンズオン（9-5-5）で行います。
...
| 行 | コード | 演算子 | 意味 |
|:---:|:---|:---:|:---|
| 1 | `$user = User::find(1);` | `::` | Userクラスの静的メソッドを呼び出し、ID=1のユーザーを取得 |
| 2 | `$posts = $user->posts;` | `->` | インスタンスのプロパティ（リレーション）にアクセス |
| 4 | `foreach ($posts as $post)` | - | 取得した投稿を1件ずつ処理 |
| 5 | `echo $post->title;` | `->` | 各投稿インスタンスのtitleプロパティにアクセス |

> **💡 Tutorial 7の復習**: `$user->posts`は、Tutorial 7で学んだ「オブジェクトのプロパティにアクセスする」構文と同じです。ただし、Laravelでは`posts`という名前のプロパティではなく、`posts()`メソッドの結果が自動的に取得されます。これを**動的プロパティ**と呼びます。
...
`$user->posts()->where('is_published', true)->get()` を分解してみましょう。

| ステップ | コード | 演算子 | 戻り値 | 意味 |
|:---:|:---|:---:|:---|:---|
| 1 | `$user->posts()` | `->` | クエリビルダ | リレーションメソッドを呼び出し |
| 2 | `->where('is_published', true)` | `->` | クエリビルダ | 条件を追加 |
| 3 | `->get()` | `->` | Collection | 結果を取得 |

> **💡 ポイント**: `posts()`と`posts`の違いに注意してください。
> - `$user->posts()` → **メソッド呼び出し**（`()`あり）→ クエリビルダが返る → 追加の条件を付けられる
> - `$user->posts` → **プロパティアクセス**（`()`なし）→ 結果のCollectionが返る
...
`Post::with('user')->get()` を分解してみましょう。

| ステップ | コード | 演算子 | 戻り値 | 意味 |
|:---:|:---|:---:|:---|:---|
| 1 | `Post::with('user')` | `::` | クエリビルダ | Postクラスの静的メソッドを呼び出し、userリレーションを事前読み込み |
| 2 | `->get()` | `->` | Collection | 結果を取得 |

> **💡 ポイント**: `Post::with()`は**静的メソッド**なので`::`を使います。その戻り値（クエリビルダ）に対して`->get()`を呼び出しています。
...
この複雑なメソッドチェーンを1つずつ分解してみましょう。

| ステップ | コード | 演算子 | 戻り値 | 意味 |
|:---:|:---|:---:|:---|:---|
| 1 | `Post::with('user')` | `::` | クエリビルダ | userリレーションを事前読み込み |
| 2 | `->where('is_published', true)` | `->` | クエリビルダ | 公開済みのみに絞り込み |
| 3 | `->orderBy('created_at', 'desc')` | `->` | クエリビルダ | 新しい順に並び替え |
| 4 | `->paginate(10)` | `->` | LengthAwarePaginator | 10件ずつページネーション |

> **💡 メソッドチェーンの仕組み**: 各メソッドが「クエリビルダ」を返すので、`->`で次のメソッドを呼び出せます。最後の`paginate()`で実際にSQLが実行され、結果が返されます。
