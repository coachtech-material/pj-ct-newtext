...
例えば、ユーザーが投稿フォームに長い文章を入力して送信ボタンを押したとき、タイトルが未入力などの理由でバリデーションエラーになり、元のフォーム画面に戻されたとします。このとき、入力した値が消えていると、ユーザーは最初からすべて入力し直す必要があり、大きなストレスになります。`old()`を使って入力値を再表示すれば、ユーザーはエラーになった箇所だけを修正して、すぐに再送信できます。
...
編集フォームでは、既存のデータをデフォルト値として表示したい場合があります。例えば、すでに登録済みのアカウント情報（ユーザー名やメールアドレスなど）を修正する画面をイメージしてください。編集画面を開いたときには「今の登録内容」が最初から表示されている必要があり、これがデフォルト値にあたります。
