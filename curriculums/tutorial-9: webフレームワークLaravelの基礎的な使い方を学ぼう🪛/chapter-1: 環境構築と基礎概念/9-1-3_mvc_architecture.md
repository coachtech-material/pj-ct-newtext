...
**お客さん（ブラウザ）** が **ホールスタッフ（ルーティング）** に「ユーザー一覧」を注文します。
...
**ホールスタッフ（ルーティング）** が、注文内容を確認し、適切な **厨房リーダー（コントローラー）** に伝えます。
...
**厨房リーダー（コントローラー）** が、部下の **シェフ（モデル）** に指示を出します。

```
厨房リーダー: 「ユーザー一覧の注文だ。シェフ、ユーザーデータを全部取ってきてくれ」
```

厨房リーダーは**司令塔**です。自分では調理（データ処理）も盛り付け（HTML生成）もしません。部下に指示を出すだけです。
...
**厨房リーダー（コントローラー）** は、シェフから受け取った料理を **盛り付け担当（ビュー）** に渡します。
...
盛り付けが完了した料理（HTML）を、**厨房リーダー（コントローラー）** が **ホールスタッフ（ルーティング）** に渡し、ホールスタッフがお客さんの元へ運びます。
...
{
    // 今回は「全員分取ってきて」という基本的な指示なので、シェフは特別な調理法を書く必要がない

    // 例えば「VIPだけ」「今月入会した人だけ」のような特別な指示が必要な場合は、このファイルに書き足していきます。
    // 今回は「VIPのお客様だけ取ってきて」といった指示の場合のコードを下記に記載するので雰囲気だけ掴んでください。
...
VIPのお客様だけ取得するコードを見てみましょう。

このように書くと、コントローラーから `User::vip()->get()` で「VIPのお客様だけ」を取得できます。

高級レストランで言えば、**シェフが「VIP専用の調理法」を覚えた**ようなものです。

> 💡 **ポイント**: 「設定より規約」の例  
> `User`モデルを作ると、Laravelは自動的に`users`テーブルと紐づけます。「Userモデルはusersテーブルを使う」という**規約**があるからです。  
> 「このモデルはこのテーブルを使う」と設定を別途書く必要はありません。（後ほど学びます。）  
...
それぞれの専門性を持った人がいて、その人材を追加・変更することが容易であることが、長く続くお店の秘訣です。
