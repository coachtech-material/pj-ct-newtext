...
```
~/laravel-practice/
├── 9-1-8_hands-on/                       ← このハンズオン用のディレクトリ
...
**この演習で作るもの**：
コントローラーで固定データを準備し、Bladeビューに渡して表示する「自己紹介ページ」を作成します。
...
- `/profile`にアクセスすると、自己紹介ページが表示される
- 以下のデータが表示される（コントローラー内で固定値として準備）
...
- [ ] 名前「山田太郎」が表示される
- [ ] 年齢「25歳」が表示される
- [ ] 趣味がリスト形式で表示される（プログラミング、読書、旅行）

> 💡 **動作確認**: `http://localhost/profile` にアクセス

---

### ✏️ 実装タスク

1. `ProfileController`を作成する
2. ルーティングを定義する（`/profile` → `ProfileController@index`）
3. コントローラーにデータを準備してビューに渡すロジックを実装する
4. `profile.blade.php`ビューを作成する
...
```bash
# ハンズオンディレクトリに移動
cd ~/laravel-practice/9-1-8_hands-on
...
> **📌 コマンドの実行場所**
>
> `sail artisan`コマンドは、**Laravelプロジェクトのルートディレクトリ**（`compose.yaml`があるディレクトリ）で実行する必要があります。
> 環境準備で移動したプロジェクトディレクトリにいることを確認してください。
...
ブラウザで `http://localhost/profile` にアクセスして、プロフィールページが表示されれば完成です！
