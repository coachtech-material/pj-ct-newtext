...
前のセクションの高級レストランの比喩で言えば、**ホールスタッフがお客様の注文を受け取り、厨房リーダー（コントローラー）に伝える**のがルーティングです。
...
ルーティングは、ユーザーのリクエストを正しいコントローラーに届ける「道案内」の役割を担っています。

<img alt="9-1-4_c1.png" src="">
...
ここまで学んだルーティングとビューがどう繋がるかを、コード例で確認します。
...
> **📌 コマンドの実行場所**
> 
> `sail artisan`コマンドは、**Laravelプロジェクトのルートディレクトリ**（`docker-compose.yml`があるディレクトリ）で実行します。

```bash
sail artisan route:list
```

このコマンドを実行した場合、現在定義されている全てのルーティングが表示されます。
