...
このチャプターでは、マイグレーション、シーダー、ファクトリーを使ったデータベース管理の仕組みを学びます。コマンドやコードが多く出てきますが、まずは「こういう手順でデータベースを構築するんだな」と理解しながら読み進めてください。実際にコマンドを実行してデータベースを構築するのは、チャプターの最後にあるハンズオン（9-3-9）で行います。
...
このコマンドを実行した場合、`database/migrations/`ディレクトリ内の全てのマイグレーションファイルが順番に実行され、テーブルが作成されます。
