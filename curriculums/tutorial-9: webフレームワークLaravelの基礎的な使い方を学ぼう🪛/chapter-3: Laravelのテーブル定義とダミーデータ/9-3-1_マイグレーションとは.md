...
Laravelでは、マイグレーションファイルを`database/migrations/`ディレクトリに配置し、`sail artisan migrate`コマンドで実行します。
...
マイグレーションファイルをGitで管理することで、他の開発者は`git pull`でファイルを取得し、`sail artisan migrate`を実行するだけで、同じテーブル構造を再現できます。
...
マイグレーションには、テーブルを作成する`up()`メソッドと、テーブルを削除する`down()`メソッドの両方が定義されています。これにより、`sail artisan migrate:rollback`コマンドで、変更を元に戻すことができます。
...
*   `sail artisan migrate`でマイグレーションを実行し、`sail artisan migrate:rollback`でロールバックできる。
