...
> 💡 **TIP**: 生のSQLを使ったトランザクションは、SQL文の動作をそのまま確認したい場合に役立ちますが、実務では`DB::transaction()`や`DB::beginTransaction()`を使う方が安全で簡潔です。Laravelが提供するメソッドを使うことで、ロールバック処理の書き忘れを防ぎ、コードの可読性も向上します。
...
次のセクションでは、ここまでのChapter 3で登場したLaravelの構文を整理するコラムと、マイグレーション・シーダー・クエリビルダを実際に使うハンズオン演習に進みます。
