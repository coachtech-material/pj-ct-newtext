...
> **💡 確認ポイント**: PRを作成したら、「Files changed」タブでモデルの実装を確認してみましょう。`$fillable` の設定やリレーションメソッドが正しく定義されているかを確認することで、実装の品質を担保できます。
...
# マージ済みのブランチを削除
git branch -d feature/issue-2-models
```
