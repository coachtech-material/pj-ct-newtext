...
> **📌 補足**: 所有者チェック（`$task->user_id !== auth()->id()`）は、次のセクション「16-6-3 タスクPolicy実装」でPolicyを使った認可に置き換えます。

---

### ステップ5: ルーティングの設定

`routes/web.php` を編集し、16-6-1で残していたタスクの仮ルートを本実装に置き換えます。

> **📌 ポイント**: 16-6-1で `Route::get('/tasks', fn() => 'タスク一覧（準備中）')` として仮定義したルートを削除し、リソースルートに置き換えます。
