...
```
ご指摘ありがとうございます。
ただ、この機能はシンプルなので、コントローラーに書いても問題ないと思います。
サービスクラスに分離するメリットは何でしょうか？
```
