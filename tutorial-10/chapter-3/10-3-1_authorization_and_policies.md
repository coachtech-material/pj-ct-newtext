...
認可を学ぶ前に、「管理者かどうか」を判定するための`is_admin`カラムを`users`テーブルに追加しましょう。
...
| コード | 説明 |
|:---|:---|
| `Schema::table('users', ...)` | 既存の`users`テーブルを変更する |
| `$table->boolean('is_admin')` | 真偽値（true/false）のカラムを追加 |
| `->default(false)` | デフォルト値を`false`（一般ユーザー）に設定 |
| `->after('password')` | `password`カラムの後ろに配置 |
...
`is_admin`カラムを一括代入可能にするため、`$fillable`に追加します。
...
> 💡 **ポイント**: `is_admin`カラムは、この後のポリシーで「管理者のみ削除可能」などの権限チェックに使用します。
