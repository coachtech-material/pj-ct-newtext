...
- [ ] `command not found` エラーが出たら、まずコマンドのスペルミスを疑うことができる。
- [ ] `No such file or directory` エラーが出たら、`pwd` と `ls` を使って、現在地とファイル/ディレクトリの存在を確認できる。
- [ ] `Permission denied` エラーが出たら、それは自分が操作すべきでないファイルを触ろうとしている可能性が高い、と判断できる。
- [ ] `sudo` コマンドが、管理者権限でコマンドを実行するための強力で危険なコマンドであることを理解している。
