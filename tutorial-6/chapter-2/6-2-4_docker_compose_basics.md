...
*   `ports`: ホストOSとコンテナの、**ポートの対応付け**を設定します。例えば、`"8080:80"`と書くと、「ホストOSの8080番ポートにアクセスすると、コンテナの80番ポートに転送される」という意味になります。これにより、ブラウザで`http://localhost:8080`にアクセスすると、コンテナ内のWebサーバーに接続できるようになります。（`ホスト側:コンテナ側`の順番で書きます）
