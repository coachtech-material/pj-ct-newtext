...
- [ ] `docker run`コマンドを使って、Nginxコンテナを、バックグラウンドで起動できる。
- [ ] `docker ps`コマンドで、実行中のコンテナの一覧を表示できる。
- [ ] `docker exec`コマンドで、実行中のコンテナの、中に入ることができる。
- [ ] `docker stop`と`docker rm`を使って、コンテナを停止し、削除できる。
- [ ] `docker images`と`docker rmi`を使って、イメージを管理できる。
