...
リファクタリングが完了したら、最後に**テスト**を書きます。

### 理由1: リファクタリングで壊れていないか確認
...
| 順番 | 作業 | 理由 |
|:---:|:---|:---|
| 1 | テスト環境セットアップ | Factoryを作成 |
| 2 | CRUDテスト | 基本機能のテスト |
| 3 | 認証・認可テスト | セキュリティのテスト |
...
#### `Book::factory()->count(5)->create()`の分解

| 部分 | 説明 | 戻り値 |
|:---|:---|:---|
| `Book::factory()` | BookFactoryインスタンスを取得 | Factory |
| `->count(5)` | 5件作成するよう設定 | Factory |
| `->create()` | DBに保存してインスタンスを返す | Collection |

> 💡 **メソッドチェーンの流れ**: `factory()` → `count()` → `create()` と順番に処理が連鎖します。
