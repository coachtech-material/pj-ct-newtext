...
カテゴリー用のビューファイルを作成します。

#### 一覧ビュー（index）

`resources/views/categories/index.blade.php` を作成します。
        ...
        <table style="width: 100%; border-collapse: collapse;">
            <thead>
                <tr style="background-color: #f5f5f5;">
                    <th style="padding: 10px; text-align: left; border-bottom: 2px solid #ddd;">カテゴリー名</th>
                    <th style="padding: 10px; text-align: center; border-bottom: 2px solid #ddd;">タスク数</th>
                ...
                @foreach ($categories as $category)
                    <tr>
                        ...
                        <td style="padding: 10px; text-align: center; border-bottom: 1px solid #ddd;">
                            {{ $category->tasks_count }}件
                        </td>
                        <td style="padding: 10px; text-align: center; border-bottom: 1px solid #ddd;">
                            <a href="{{ route('categories.edit', $category) }}" class="btn btn-secondary" style="padding: 5px 10px; font-size: 0.875rem;">編集</a>
...
| コード | 説明 |
|:---|:---|
| `$categories->isEmpty()` | コレクションが空かどうかを確認 |
| `@foreach ($categories as $category)` | カテゴリーをループ処理 |
| `route('categories.show', $category)` | モデルを渡すとIDが自動的に使用される |
| `@method('DELETE')` | DELETEメソッドを指定（HTMLフォームはGET/POSTのみ対応のため） |
...
`resources/views/categories/create.blade.php` を作成します。
...
<div class="card" style="max-width: 600px; margin: 0 auto;">
    <h2 style="margin-bottom: 20px;">カテゴリー作成</h2>
...
`resources/views/categories/show.blade.php` を作成します。
...
<div class="card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h2>{{ $category->name }}</h2>
        <a href="{{ route('categories.index') }}" class="btn btn-secondary">一覧に戻る</a>
    </div>

    <h3 style="margin-bottom: 15px;">このカテゴリーのタスク（{{ $category->tasks->count() }}件）</h3>

    @if ($category->tasks->isEmpty())
        <p>このカテゴリーにはタスクがありません。</p>
    @else
        <ul style="list-style: none;">
            @foreach ($category->tasks as $task)
                <li style="padding: 10px; border-bottom: 1px solid #ddd;">
                    <a href="{{ route('tasks.show', $task) }}">{{ $task->title }}</a>
                    <span style="color: #666; font-size: 0.875rem;">（優先度: {{ $task->priority_label }}）</span>
                </li>
            @endforeach
        </ul>
    @endif
...
`resources/views/categories/edit.blade.php` を作成します。
...
@section('content')
<div class="card" style="max-width: 600px; margin: 0 auto;">
    <h2 style="margin-bottom: 20px;">カテゴリー編集</h2>
...
| コード | 説明 |
|:---|:---|
| `@method('PUT')` | PUTメソッドを指定（更新処理用） |
| `old('name', $category->name)` | バリデーションエラー時は入力値、それ以外は既存値を表示 |
